...
public class ViewChart extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        back.setOnClickListener(vw -> {
            Intent backIntent = new Intent(vw.getContext(), MainActivity.class);
    ...
    public  void getJSON() throws FileNotFoundException {
        ...
        for (int i = 0; i < yPrices1.size(); i++) {
            yValues1.add(new Entry((float) i, yPrices1.get(i)));
        }

        for (int i = 0; i < yPrices2.size(); i++) {
            yValues2.add(new Entry((float) i, yPrices2.get(i)));
        }
